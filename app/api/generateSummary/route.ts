...
const buildMessages = (todos: { [key in TypedColumn]: number }): { role: 'system' | 'user', content: string }[] => {
    return [{
        role: 'system',
        content: `回复时，始终称呼用户为刘先生，并说欢迎使用Todo App！回复内容不能超过200个字符。`
    }, {
        role: "user",
        content: `嗨，这里是所有待办事项的摘要。计数每个类别中有多少个待办事项，例如待办、进行中和已完成，然后告诉用户祝他过一个充实的一天！以下是数据：${JSON.stringify(todos)}`
    }]
}
